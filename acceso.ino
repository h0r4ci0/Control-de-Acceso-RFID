...
int statusButtonBlue;
int statusButtonYellow;
...
void loop() {
    ...
    // LLAVERO
    if (comparaUID(LecturaUID, Key) && statusButtonBlue == HIGH) {
        ...
        lcd.backlight();
        lcd.setCursor(0,0);
        lcd.print("Bienvenido Horacio!");
        delay(1000);
        lcd.clear();
        lcd.print("nvenido Horacio!");
        delay(4000);

        lcd.clear();
        lcd.print("El saldo es de:");
        lcd.setCursor(0,1);
        lcd.print(saldo);
        lcd.print(" EUR");
        delay(5000);
    ...
    // CARD
    if (comparaUID(LecturaUID, Card) && statusButtonBlue == HIGH){
    ...
    // SALDO - MENÚ
    statusButtonBlue = digitalRead(button_blue);        // comprueba si el botón azul está pulsado
    statusButtonYellow = digitalRead(button_yellow);    // comprueba si el botón amarillo está pulsaod

    if (comparaUID(LecturaUID, Key) or comparaUID(LecturaUID, Card) && statusButtonBlue == LOW) {
        lcd.clear();
        lcd.backlight();
        lcd.setCursor(0,0);
        lcd.print("----- MENU -----");
        lcd.setCursor(0,1);
        lcd.print("1. Consultar Saldo");
        delay(1000);
        lcd.clear();
        lcd.backlight();
        lcd.setCursor(0,0);
